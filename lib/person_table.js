window.onload = function () {
  ...
  function legendClick(e){
    ...
    if (typeof(e.dataSeries.visible) === "undefined" || e.dataSeries.visible) {
      if (tokutaiseiNum - 1 == 0) {
        alert('グラフを全非表示にはできません');
        return;
      } else {
  ...
  $('input[name="graph-from"]').on('change', function() {
    var selectDateSplit = $('input[name="graph-from"]').val().split('-');
    var otherDateSplit = $('input[name="graph-to"]').val().split('-');
    if (Number(selectDateSplit[0] + selectDateSplit[1]) >= Number(otherDateSplit[0] + otherDateSplit[1])) {
      alert('正しい日付を選択してください');
    } else {
      var selectDateMiliTime = new Date(selectDateSplit[0], String(Number(selectDateSplit[1])-1), "1").getTime();
      dateChart.axisX[0].set("minimum", selectDateMiliTime);
    }
  });
  $('input[name="graph-to"]').on('change', function() {
    var selectDateSplit = $('input[name="graph-to"]').val().split('-');
    var otherDateSplit = $('input[name="graph-from"]').val().split('-');
    if (Number(selectDateSplit[0] + selectDateSplit[1]) <= Number(otherDateSplit[0] + otherDateSplit[1])) {
      alert('正しい日付を選択してください');
    } else {
      var selectDateMiliTime = new Date(selectDateSplit[0], Number(selectDateSplit[1]), "0").getTime();
      dateChart.axisX[0].set("maximum", selectDateMiliTime);
    }
  });
