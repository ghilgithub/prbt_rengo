...
$(function() {
  ...
  $.getJSON('output.json', function(allPdata){
    ...
    if (pData.kettei != '' || pData.botu != '') {
      $('#table-person-info').append('<tr><td class="nowrap">俳句傑作認定</td><td id="kushu-info"></td></tr>');
      if (pData.kettei != '') {
        $('#kushu-info').append('<span class="nowrap"><span class="label label-up">掲載決定</span>&nbsp;× ' + pData.kettei + '&nbsp;&nbsp;&nbsp;&nbsp;</span> ');
      }
      if (pData.botu != '') {
    ...
    $.each(hDataArray, function(index, elem) {
      ...
      } else if (elem.結果_試験 == '掲載決定') {
        hResult = '<span class="label label-up">掲載決定</span>&emsp;' + elem.段位 + ' / 50句'
      } else if (elem.結果_試験 == 'ボツ') {
        hResult = '<span class="label label-down">ボツ</span>'
      } else if (titleResult != '') {
        hResult = titleResult;
      } else {
        hResult = '<b>' + elem.結果 + '</b>';
